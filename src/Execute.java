...
public class Execute {
	...
	public static void Run(MarketMaker m) {
	 ...
	 // to read each data set
	 	int df=1;
	  //**/for(   df=1; df<U.dataFileArr_UCI.length ; df++)
		{

		//	String dataFile= U.testFile;
			...
			//  for (    U.Qalpha=0.1 ; U.Qalpha < 1; U.Qalpha +=  0.2)
				{

				  U.ExperimentNote  =fileInfo+ " Qalpha="+ U.Qalpha ;
				  
				// to experiment for different days (i.e number of rounds)
				for (int days = U.minDay; days <= U.maxDay; days += U.daySteps) {
					U.days = days;
 
					U.fileInfo+= "-rounds="+days+"-Qalpha= "+U.Qalpha;
					...
					// Registering all the agents to the market
					mm.removeAllAgents();
	...
	public static int runExperiment(DataDistributer myDataDistributer,String outMA) {
		...
		// to store correct answer and market prediction into history

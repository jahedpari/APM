...
public class QlearningRegTradeStaretgy extends TradeStrategy_Regression {
	...
	public HashMap<Double, Double> giveBasket(RegressionAgent agent) {
		...
		if (U.marketNumber == 1) {
			...
			if (U.useConfidence == false)
				...
				bid = Math.min(agent.getConfidence() * agent.getCapital(),
						maxMoney);
	...
	public void updateBelief(double previousReward,
		...
		for (int day = 2; day <= U.days; day++) {
			...
			if (alpha.compareTo(new BigDecimal(0, MathContext.DECIMAL32)) == -1) {
				alpha = new BigDecimal(0, MathContext.DECIMAL32);
				WriteToReportRL("alpha is less than one => alpha: " + alpha);
			}
			//store the new alpha in the Q-table
		//	Q[stateIndex][alphaIndex] =   alpha;
			 
		BigDecimal alphaTemp1=	new BigDecimal((1 - U.Qalpha), MathContext.DECIMAL32);
			BigDecimal alphaTemp2=	new BigDecimal(  U.Qalpha , MathContext.DECIMAL32);	
		 Q[stateIndex][alphaIndex] =   alphaTemp1.multiply(Q[stateIndex][alphaIndex]).add( alphaTemp2.multiply(alpha) );
			...
			for (int j = action1Index; j < numberOfColumns; j++) {
				...
				//if alpha is zero, then use first action
				if(actionIndex==2 && alpha.compareTo(new BigDecimal(0, MathContext.DECIMAL32)) == 0)
				{
					newQ=0;
				}
				...
				output += " old Qvalue="+ Q[stateIndex][j].doubleValue()+ "  U.Qalpha:  "+U.Qalpha ;
				output += "\n (1 - U.Qalpha)* Q[stateIndex][j].doubleValue() =>"+ (1 - U.Qalpha)+ "  *"+Q[stateIndex][j].doubleValue() ;
				output += "\n (U.Qalpha) * revenue => "+ (U.Qalpha)+"*" + revenue+ "=> "+ (U.Qalpha) * revenue+ "\n";
				output += "\n newQ: "+newQ;
	...
	// finds the state based on agent prediction difference with marke prediction
	...
	public int argmax(int stateIndex) {
		...
		int actionIndexWithMaxQ = firstActionIndex;
		...
		for (int j = firstActionIndex ; j < numberOfColumns; j++) {
			 U.p("\n **** action: "+j+ "   Q[j]: "+ Q[stateIndex][j] );
			if (Q[stateIndex][j].compareTo(maxQ) == 1) // if bigger
			{
				maxQ = Q[stateIndex][j];
				actionIndexWithMaxQ = j;

			}

		}

		actionIndexWithMaxQ = actionIndexWithMaxQ - firstActionIndex;
	 U.p("\n ***** actionIndexWithMaxQ: "+actionIndexWithMaxQ);
		...
		for (int j = firstActionIndex; j < numberOfColumns; j++) {
			...
			if (Q[stateIndex][j].compareTo(maxQ) == 0) // if equal
			{
				...
				output += " \n  action   " + (j - firstActionIndex)
						+ "  is equal to action " + actionIndexWithMaxQ + " \n";
			}
		...
		if (equalIndex.size() > 1) {
			...
			actionIndexWithMaxQ = MaxActionNo;

			// for reporting purpose
			output += " \n most frequent action ( " + actionIndexWithMaxQ
					+ ") is chosen out of  :" + equalIndex.size()
					+ "  number of action with same Q value";
			WriteToReportRL("\n most frequent action ( " + actionIndexWithMaxQ
					+ ") is chosen out of  :" + equalIndex.size()
					+ "  number of action with same Q value");
			output += " \n =>   best action: " + (actionIndexWithMaxQ + 1);
			
			WriteToReportRL(output);
		}

		return (actionIndexWithMaxQ + 1);
	}
